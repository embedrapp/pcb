...
pub trait LspContext {
    ...
    /// Return rich completion metadata (type, docs, etc.) for the given
    /// symbol as it is visible from `current_file`.  Implementations can
    /// return `None` when no extra information is available.
    fn get_completion_meta(&self, _current_file: &LspUrl, _symbol: &str) -> Option<CompletionMeta> {
        None
    }
    ...
    /// import completions, etc.) work without the user having to open each file first.
    ///
    /// The default implementation returns `true`, enabling the behaviour unless an
    /// implementation explicitly opts out.
    fn is_eager(&self) -> bool {
        true
    }

    /// Provide the list of files that should be parsed when [is_eager] is `true`.
    /// Implementations can decide which files are relevant for their dialect.
    /// The paths **must** be absolute because they will be converted into `file://` URLs.
    ///
    /// The default implementation returns an empty list, meaning "let the server skip
    /// eager parsing".
    fn workspace_files(
        &self,
        _workspace_roots: &[std::path::PathBuf],
    ) -> anyhow::Result<Vec<std::path::PathBuf>> {
        Ok(Vec::new())
    }

    /// Check if there is a module dependency between two files.
    fn has_module_dependency(&self, _from: &Path, _to: &Path) -> bool {
        false
    }

    /// Provide custom hover information for a loaded symbol.
    ///
    /// This hook allows implementations to handle special cases like directory imports
    /// or provide custom documentation. Return `None` to fall back to the default behavior.
    ///
    /// `load_path` is the path string from the load() statement
    /// `symbol_name` is the name of the symbol being hovered
    /// `current_file` is the file containing the load() statement
...
impl<T: LspContext> Backend<T> {
    ...
    fn validate(&self, uri: Url, version: Option<i64>, text: String) -> anyhow::Result<()> {
        ...
        // kept up-to-date without requiring the user to touch them.
    ...
    fn resolve_definition_location(
        ...
        let ret = match definition {
            IdentifierDefinition::Location {
                ...
                if let Some(ext_uri) = self.context.get_url_for_global_symbol(uri, &name)? {
                    // Try to pinpoint a more accurate position within the external file.
                    let ext_range = self
                        .get_ast_or_load_from_disk(&ext_uri)?
                    ...
                    Self::location_link(source, &ext_uri, ext_range)?
                } else {
                    // Fallback to the original behaviour (local jump).
    ...
    fn get_hover_for_identifier_definition(
        ...
        Ok(match identifier_definition {
            ...
            IdentifierDefinition::LoadedLocation {
                ...
                // First try the custom hover handler
                if let Some(custom_hover) = self.context.get_custom_hover_for_load(
                    &path,
                    &name,
                    document_uri,
                    workspace_root,
                )? {
                    return Ok(Some(custom_hover));
                }

                // Fall back to default behavior: Symbol loaded from another file
    ...
    /// Re-validate all open documents that (transitively) depend on `changed`.
    fn propagate_change(&self, changed: &LspUrl) -> anyhow::Result<()> {
        ...
        let mut visited: HashSet<LspUrl> = HashSet::new();
        ...
        while let Some(current) = queue.pop_front() {
            for uri in &docs {
                // Skip self and already-visited nodes.
                if uri == &current || visited.contains(uri) {
                    continue;
                }
                ...
                if depends_on_current {
                    // Re-validate the dependent file so its diagnostics refresh.
                    self.quick_validate(uri)?;

                    // Enqueue for further propagation (transitive deps).
    ...
    /// Lightweight validation used during dependency propagation. Parses the
    /// current contents (either the in-memory buffer or on-disk) of `uri`,
    ...
    /// When a load() statement refers to a *directory* and not a file we try to resolve the
    /// requested module name to a file inside that directory (e.g. `load("dir", "foo")`
    /// might resolve to `dir/foo.star`).  This helper searches the directory for the most
    /// likely candidate and returns a `LocationLink` pointing at the file.
    fn find_module_in_directory(
        source: ResolvedSpan,
        dir: &Path,
        module_name: &str,
    ) -> anyhow::Result<Option<LocationLink>> {
...
pub fn server_with_connection<T: LspContext>(
    ...
    let server_settings = initialization_params
        ...
        .unwrap_or_default();
    let capabilities_payload = Backend::<T>::server_capabilities(&context, server_settings);
...
/// Rich metadata about a symbol used to enhance completion items.
#[derive(Debug, Clone)]
pub struct CompletionMeta {
    pub kind: Option<CompletionItemKind>,
    pub detail: Option<String>,
    pub documentation: Option<String>, // markdown
}
