...
function App() {
  ...
  const handlePositionsChange = useCallback(
    (componentId: string, positions: any) => {
      ...
      if (vscode) {
        ...
        const savePositionsUrl = params.get("savePositions");

        if (!savePositionsUrl) {
          console.log(
            `[App] Not in VS Code environment and no savePositions endpoint was provided, skipping position update`
          );
          return;
        }

        fetch(savePositionsUrl, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ componentId, positions }),
        }).catch((error) => {
  ...
  const loadPositions = useCallback(
    async (componentId: string): Promise<any> => {
      ...
      if (!vscode) {
        const params = new URLSearchParams(window.location.search);
        const positionsUrl = params.get("positions");

        if (!positionsUrl) {
          console.log(
            `[App] Not in VS Code environment and no positions endpoint was provided, returning null`
          );
          return null;
        }

        const url = new URL(positionsUrl, window.location.href);
      ...
      return new Promise((resolve) => {
        ...
        setTimeout(() => {
          console.log(
            `[App] loadPositions request timed out for: ${requestId}`
          );
          window.removeEventListener("message", handler);
          resolve(null);
        }, 5000);
  ...
  useEffect(() => {
    if (vscode) {
      ...
      // Browser environment - load a netlist from ?netlist=... when provided,
      // otherwise use the bundled demo data.
      const params = new URLSearchParams(window.location.search);
      const netlistUrl = params.get("netlist");
      ...
      if (netlistUrl) {
        fetch(netlistUrl)
          .then((response) => {
            if (!response.ok) {
              throw new Error(
                `Failed to load ${netlistUrl}: ${response.status} ${response.statusText}`
              );
            }
            return response.json();
          })
          .then((netlist) => {
            if (!isValidNetlist(netlist)) {
              throw new Error(`${netlistUrl} did not contain a valid netlist`);
            }

            const rootRef = netlist.root_ref;
            const currentFile =
              params.get("currentFile") ||
              (rootRef ? rootRef.split(":<root>")[0] : undefined);

            setNetlistData(netlist);
            setCurrentFile(currentFile);
            setSelectedModule(params.get("selectedModule") || rootRef);
            setIsLoading(false);
          })
          .catch((error) => {
            console.error(error);
